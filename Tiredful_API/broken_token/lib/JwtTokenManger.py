...
class JwtToken:
	...
	def decode_user_jwt_token(self):
		user_info = ''
		try:
			user_info = jwt.decode(self.payload, options={"verify_signature": False})
		except:
			return False
	...
	def decode_user_jwt_token_hs256(self):
		...
		try:
			if signing_algo.upper() == 'RS256':
				user_info = jwt.decode(self.payload, self.__get_pub_key__(), algorithms=['RS256'])
			elif signing_algo.upper() == 'HS256':
				user_info = jwt.decode(self.payload, self.__get_fake_pub_key__(), algorithms='HS256')
			else:
				return False
		except:
			print('exp')
